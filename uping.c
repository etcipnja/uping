...
void print_stats()
{
    ...
    fprintf(stderr,"rtt min/avg/max = %.1f/%.1f/%.1f\n\n",stats.min_diff/10.0,
            stats.avg_diff/10.0,stats.max_diff/10.0);
        ...
        switch(c)
        {
            case -1:
                if (arguments.host==NULL)
                {
                    ...
                    {
                        printf("failed to resolve remote socket address (err=%s)\n",strerror(errno));
                        exit(1);
                    }
...
void do_server()
{
    ...
    else if (retval)
    {
        ...
        char buffer[1024];
        long size=recvfrom(var.fd, &buffer[0], sizeof(buffer),0, &dest, &dest_size);
...
int do_client()
{
    ...
    else if (retval) //reply received
    {
        ...
        long size=recv(var.fd, &buffer[0],sizeof(buffer),0);
        if (var.stop_signal==1) return 0;
        if (-1==size)
        {
        ...
        if (m==var.magic)
        {
            ...
            {
                stats.received++;
                int diff=(int)(now()-var.packets[i].sent_ts);
                if (diff<stats.min_diff || stats.min_diff==0) stats.min_diff=diff;
                if (diff>stats.max_diff || stats.max_diff==0) stats.max_diff=diff;
                if (stats.avg_diff==0) stats.avg_diff=diff;
                else { stats.avg_diff+=diff; stats.avg_diff/=2; }
                if (!arguments.silent)
                {
                    printf("%ld bytes from %s:%i sequence %i, time %.1f ms\n",
